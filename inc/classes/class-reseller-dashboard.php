...
class Reseller_Dashboard {
    ...
    public function render_dashboard_layout() {
        ...
        // My Shop sidebar item opens the public storefront (settings live under Settings).
        if ( isset( $tabs[ $tab ]['type'] ) && 'shop_link' === $tabs[ $tab ]['type'] ) {
            ...
            <aside class="rm-dashboard-sidebar">
                ...
                <nav class="rm-sidebar-nav">
                    ...
                    foreach ( $tabs as $tab_key => $tab_data ) : 
                        ...
                        $is_shop_link = isset( $tab_data['type'] ) && 'shop_link' === $tab_data['type'];
                        $is_active    = ! $is_shop_link && ( $tab_key === $tab );
                        $has_children = ! empty( $tab_data['children'] );
                        $nav_href     = $is_shop_link
                            ? Reseller_Helper::get_shop_url( $user_id )
                            : $this->get_dashboard_tab_url( $tab_key );
                        ?>
                        <div class="rm-nav-item-wrapper <?php echo $is_active ? 'is-active' : ''; ?> <?php echo $has_children ? 'has-children' : ''; ?> <?php echo $is_active ? 'is-expanded' : ''; ?>">
                            <a class="rm-nav-link <?php echo $is_active ? 'is-active' : ''; ?>" href="<?php echo esc_url( $nav_href ); ?>"<?php echo $is_shop_link ? ' target="_blank" rel="noopener noreferrer"' : ''; ?>>
    ...
    public function get_svg_icon( $name ) {
        $icons = [
            ...
            'my-shop'   => '<path d="M20 4H4v2h16V4zm1 10v-2l-1-5H4l-1 5v2h1v6h10v-6h4v6h2v-6h1zm-9 4H6v-4h6v4z"/>',
